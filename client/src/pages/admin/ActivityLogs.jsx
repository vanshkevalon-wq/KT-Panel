...
import { FiActivity, FiSearch, FiFilter, FiClock } from 'react-icons/fi';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">System Activity Logs</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search activity description or user email..."
              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log._id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="px-6 py-4 font-bold text-white whitespace-nowrap">{log.userEmail}</td>
                    <td className="px-6 py-4">
                      <Badge variant={log.userRole}>{log.userRole}</Badge>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="font-bold text-indigo-400">{log.action}</span>
                      <span className="text-[10px] text-slate-500 block uppercase">{log.module}</span>
                    </td>
                    <td className="px-6 py-4 text-slate-200">{log.description}</td>
                    <td className="px-6 py-4 font-mono text-[11px] text-slate-500">{log.ipAddress}</td>
                  </tr>
                ))
              )}
            </tbody>
